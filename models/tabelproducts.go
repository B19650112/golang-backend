...
func ListTblProduct(db *sql.DB, tabelprd string) []entities.TabelProduct {
	...
	for rows.Next() {
		err = rows.Scan(&tbl.ID, &tbl.ImagePath, &tbl.Title, &tbl.Description, &tbl.Price, &tbl.IDTemp)
		if err != nil {
			panic(err)
		}
		res = append(res, tbl)
...
// ViewTblProduct : List tabelproduct by id
func ViewTblProduct(db *sql.DB, tabelprd string) []entities.TabelProduct {
